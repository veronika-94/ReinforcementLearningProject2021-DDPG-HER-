...
if __name__ == '__main__':
    ...
    figure_file = './plots/fetchpush.png'
        ...
        if not done:
            ...
            if abs(sum_) > 0.0005: ## entering her 
               # for _ in range(16):
                ...
                for _ in range(24):
                    puck_rand = np.copy(transition[48-_][5])
                    #end_eff_pos = transition[45-_][3][:3]
                    if  np.linalg.norm(puck_rand - goal, axis=-1) <= np.linalg.norm(puck_rand - initial_puck, axis=-1):
                         print('                                                                                        yeah!!')
                         agent.remember(transition[48-_][0], transition[48-_][1], 0,
                                               transition[48-_][3], True, puck_rand)
                         agent.learn()
                         break
                
                    agent.remember(transition[48-_][0], transition[48-_][1], transition[48-_][2],
                                               transition[48-_][3], False, puck_rand)
                    agent.learn()
