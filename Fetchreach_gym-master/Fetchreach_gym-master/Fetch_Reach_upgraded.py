...
tf.compat.v1.set_random_seed(seeding)
env.seed(seeding)
######################################
...
class actor():
    def __init__(self, state_size, action_size, action_bound, sess, ac_lr = 0.0001, tau = 0.001):
        ...
        with tf.compat.v1.variable_scope('actor_net'):

            self.input_actor, self.out_, self.scaled_out = self.actor_model()

        self.ac_params = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.GLOBAL_VARIABLES, scope='actor_net')


        with tf.compat.v1.variable_scope('actor_target_net'):
            self.input_target_actor, self.target_out_, self.target_scaled_out = self.actor_model()

        self.ac_target_pram = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.GLOBAL_VARIABLES, scope = 'actor_target_net')
        ...
        self.critic_grad = tf.compat.v1.placeholder(tf.float32,[None, self.action_size], name = 'critic_grad')

        self.actor_grad = tf.gradients(ys=self.scaled_out, xs=self.ac_params, grad_ys=-self.critic_grad)
        self.actor_gradients = list(map(lambda x: tf.compat.v1.div(x, self.batch_size), self.actor_grad))

        self.loss = tf.compat.v1.train.AdamOptimizer(self.lr).apply_gradients(zip(self.actor_gradients, self.ac_params))

    def actor_model(self):
        inputs = tf.compat.v1.placeholder(tf.float32, [None, self.state_size])
        x = tc.layers.layer_norm(inputs, center=True, scale=True, begin_norm_axis=0)
        h1 =  tf.compat.v1.layers.dense(x, 400, activation = tf.nn.relu )
        h1 = tc.layers.layer_norm(h1, center=True, scale=True)
        h2 = tf.compat.v1.layers.dense(h1, 300, activation = tf.nn.relu )
        h2 = tc.layers.layer_norm(h2, center=True, scale=True)
        k_init = tf.compat.v1.random_uniform_initializer(minval=-0.003, maxval=0.003)
        out = tf.compat.v1.layers.dense(h2, self.action_size, activation = tf.nn.tanh,kernel_initializer = k_init)
...
class critic():
    def __init__(self, state_size, action_size, action_bound, sess, ac_lr = 0.001, tau = 0.001):
        ...
        with tf.compat.v1.variable_scope('critic_net'):
            self.input_critic, self.action_critic, self.value,  = self.build_net()
        self.cr_prams = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.GLOBAL_VARIABLES, scope = 'critic_net')

        with tf.compat.v1.variable_scope('target_critic_net'):
            self.input_target_critic, self.action_target_critic, self.target_value = self.build_net()
        self.target_cr_prams = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.GLOBAL_VARIABLES, scope = 'target_critic_pram')
        ...
        self.pred = tf.compat.v1.placeholder(tf.float32, [None, 1], name= 'pred_value')
        self.loss = tf.reduce_mean(input_tensor=tf.square(self.pred - self.value))
        self.optimize = tf.compat.v1.train.AdamOptimizer(self.lr).minimize(self.loss)
        self.comment_grad = tf.gradients(ys=self.value, xs=self.action_critic)
    ...
    def build_net(self):
        ...
        action = tf.compat.v1.placeholder(tf.float32, [None, self.action_size])
        h1 = tf.compat.v1.layers.dense(x, 200, activation = tf.nn.relu)
        h1 = tc.layers.layer_norm(h1, center=True, scale=True)
        h11 = tf.compat.v1.layers.dense(h1, 200,activation = tf.nn.relu)
        a1 = tf.compat.v1.layers.dense(action, 200)

        h1_ = tf.concat([h11,a1],axis = 1)
        h1_ = tc.layers.layer_norm(h1_, center=True, scale=True)

        h2 = tf.compat.v1.layers.dense(h1_, 300, activation=tf.nn.relu)
        h2 = tc.layers.layer_norm(h2, center=True, scale=True)
        k_init = tf.compat.v1.random_uniform_initializer(minval=-0.003, maxval=0.003)
        out_cr = tf.compat.v1.layers.dense(h2, 1,kernel_initializer=k_init)
